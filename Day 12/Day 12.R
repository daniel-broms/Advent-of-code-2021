...
x <-find_paths("start", "start", vector(mode="character"))
length(x[!x==''])

#Return all paths from the given subpath. We cannot revisit visitedsmallcaves.

find_paths <- function(node, subpath, visitedsmallcaves ){
  ...
  nextnodes <- t[t[,1]==node & !t[,2]=='start' & (!t[,2] %in% visitedsmallcaves) ,]
  ...
  for(i in 1:nrow(nextnodes)){
    allsubpaths <- c(allsubpaths,find_paths(nextnodes[i,2], paste(subpath, nextnodes[i,2], sep=',' ), visitedsmallcaves))
  }
  return(allsubpaths)
}
