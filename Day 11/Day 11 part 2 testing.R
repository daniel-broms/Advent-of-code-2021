...
for(flash in 1:100000){
  ...
  while(length(t[t>9]) > 0){
    for (i in which(t>9)){
      flashes <- flashes + 1
      t[i] <- 0
      t <- flashcell(t,i%%10,ceiling(i/10))
    }
  }
  if(sum(t) == 0) break    #in part 2 : stop when all cells are zero.
}
flash   #244 : correct!

#flash a given cell : increase all surrounding cells with 1, unless they are -1.
